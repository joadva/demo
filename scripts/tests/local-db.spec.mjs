...
describe('separarSentencias', () => {
  ...
  it('divide sql/clientes.sql en la tabla, los 5 DROP y los 5 procedimientos', async () => {
    const sentencias = separarSentencias(await fs.readFile('sql/clientes.sql', 'utf8'));
    ...
    expect(sentencias.at(-1)).toMatch(/CREATE PROCEDURE sp_clientes_eliminar[\s\S]*END$/);
  });
});

describe('conexionLocal', () => {
  it('usa las credenciales del docker-compose cuando no hay variables', () => {
